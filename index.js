...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === '개집벽') {
    ...
    const message = await interaction.reply({ content: 'https://i.imgur.com/DMHtsiB.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '편지') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/5PVcuJH.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '편지2') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/Z2S8LoX.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '수양록') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/AyE0lGQ.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '수양록2') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/uXrhMQw.png', fetchReply: true });
    message.react('❤️');
    ...
    const message = await interaction.reply({ content: 'https://i.imgur.com/QfyJFtx.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '메일함') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/Thr8QvG.png', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '합격') {
    const message = await interaction.reply({ content: 'https://i.imgur.com/aBAMc11.png', fetchReply: true });
    message.react('❤️');
    ...
    const message = await interaction.reply({ content: 'https://i.imgur.com/APrRnmw.png/n', fetchReply: true });
    message.react('❤️');
    ...
    const message = await interaction.reply({ content: '```오은영 박사님과의 심리치료 프로그램이 끝이 났다. \n\n계속되는 불면증에 시달리다가 비싼 돈을 들여 예약했는데 프로그램은 끝까지 잘 참여했지만, 아직도 마음 속에 뭔가가 남아 있다. 그 전만큼 불면증이 심하진 않지만, 뭔가 찝찝하다.\n\n겨우 잠이 드나 싶은 날도 있긴 하지만, 어떤 날은 첫 애인이었던 캐니가 내 꿈에 나와서 나를 괴롭힌다. \n\n그럴때마다 스토킹 당했던 그 스무살 그 때로 돌아가는 것 같다. \n\n내가 했던 행동들은 잘 한 걸까, 아직도 의문이 든다. 뭔가 다른 방법으로 해결할 수 있지 않았을까? 모르겠다. \n\n그냥 덮어버릴래. \n\n잊어버려야하는데 왜 자꾸 다시 꿈에 나오냐고!!!!!!!!!!!!!\n\n어디서부터 잘못된 걸까.\n\n오늘도 민성이는 밤 늦게까지 눈을 감고 잠을 청해본다. \n\n언젠가는 벗어날 수 있겠지.\n\n이 고통에서_```', fetchReply: true });
    message.react('❤️');
  } else if (commandName === '구일띠') {
    ...
    setTimeout(function(){interaction.channel.send('\n\n```당신의 진짜 이름은 memory 입니다. \n\n guilty죄책감으로 얼룩진 과거를 바로잡고 리먼이와의 추억을 되찾으셨기 때문에 진짜 이름을 찾으셨습니다.``` \n\n')},10000)
    
    setTimeout(function(){interaction.channel.send('\n\n```당신은...``` \n')},4000)
    
    setTimeout(function(){interaction.channel.send('||증||')},4000)
    await interaction.channel.send('```이라는 키워드를 얻으셨습니다.```')
    
    setTimeout(function(){const message = await interaction.channel.send({ files: ["https://i.imgur.com/hoXsMBF.gif"] });},4000)
    message.react('❤️');
  } else if (commandName === '인비') {
    ...
    setTimeout(function(){const message = await interaction.channel.send({ files: ["https://i.imgur.com/3DuhauC.gif"] });},4000);
    message.react('❤️');
   ...
   setTimeout(function(){interaction.channel.send('\n\n```당신은...``` \n')},4000);
   wait(4000)
   setTimeout(function(){interaction.channel.send('||불||')},4000);
    await interaction.channel.send('```이라는 키워드를 얻으셨습니다.```')
    wait(2000)
    setTimeout(function(){const message = await interaction.channel.send({ files: ["https://i.imgur.com/ukXkrfF.gif"] });},4000);
    message.react('❤️');
  } else if (commandName === '옹루케르') {
   ...
   setTimeout(function(){interaction.channel.send('\n\n```당신의 진짜 이름은 sincere 입니다. onlooker 방관자에서 벗어나셨기 때문에 진짜 이름을 찾으셨습니다.``` \n\n')},10000);
    ...
    await interaction.channel.send('```이라는 키워드를 얻으셨습니다.```')
    wait(2000)
   setTimeout(function(){const message = await interaction.channel.send({ files: ["https://i.imgur.com/lAqxsGO.gif"] });},4000);
    message.react('❤️');
  } else if (commandName === '오프리숀') {
    ...
    await interaction.channel.send('```이라는 키워드를 얻으셨습니다.```')
    wait(2000)
    setTimeout(function(){const message = await interaction.channel.send({ files: ["https://i.imgur.com/wVWUCwO.gif"] });},4000);
    message.react('❤️');
  } else if (commandName === '약한') {
    await interaction.reply({ files: ["https://i.imgur.com/1Bt7xpH.png"] });
    const message = await interaction.channel.send({ files: ["https://imgur.com/rE7As8z.png"] });
