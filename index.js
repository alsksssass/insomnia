...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === '개집벽') {
    ...
    await interaction.deferReply();
    const message = await interaction.channel.send({ files: ["https://i.imgur.com/APrRnmw.png/n"] });
    message.react('❤️');
  ...
  await interaction.reply('```저는 스물여덟살, 오프리숀입니다. \n\n전 제 직속상사 버니님이 저를 이유없이 싫어하고 미워하고 괴롭히고 있다고 생각했어요.\n\n하지만 제 연봉을 올려주려고 힘써준 게 버니님이었다는 걸 알았어요.\n\n\n지금도 버니님이 절 억압한 방식이 옳다고는 생각하지 않아요.\n\n하지만 저는 이제 마냥 피하기만 해서는 안 된다고 생각해요.\n\n직접 버니님께 이런 부분은 감사하지만 이런 부분은 속상하다고 이야기를 드려볼까해요.\n\n단 둘이 이야기할 용기가 나지 않으면 인사팀 허니님의 도움을 받아볼까도 싶어요.\n\n\n사람들은 누구나 살면서 여러가지 고난과 역경에 부딪히게 되잖아요.\n\n그 때 혼자 끙끙 앓을 것인가, 그냥 피해버릴 것인가.\n\n아니면 문제에 직면하고 부딪혀서 돌파구를 찾아낼 것인가.\n\n나의 선택에 따라 많은 것들이 바뀔 수 있다고 믿어요.\n\n\n여러분도 여러분의 역경을 극복할 수 있길 바랍니다.```');
   ...
   const message = await interaction.channel.send('```이라는 키워드를 얻으셨습니다.```')
    ...
    if (string == wjdekq && wjsghkrkqt == 1) {
      await interaction.deferReply();
    const message = await interaction.channel.send({ files: ["뚜르르르르 뚜르르르르"] });
      message.react('📲');
    }
    if(wjsghkrkqt != 1) {
      const message1 = await interaction.reply('```폰도 없이 어떻게 전화를 걸지??```');
      message1.react('📵');
    }
    if(wjsghkrkqt == 1 && string != wjdekq) {
      const message1 = await interaction.reply('```없는 번호입니다. 다시 확인하시고 걸어주시기 바랍니다```');
      message1.react('📵');
    }
    }
    ...
    if(vpdjvhs != 1) {
      const message2 = await interaction.channel.send('```폰도 없이 어떻게 패턴을 푸나?```');
      message2.react('🔒');
    }
...
const message1 = await interaction.channel.send('```페어의 폰은 습득!```');
  ...
  message4.react('❤️');
    ...
    await interaction.deferReply();
    const message = await interaction.channel.send({ files: ["You can react with custom emojis!"] });
    message.react('👍');
  } else if (commandName === '야') {
    await interaction.deferReply();
    const message = await interaction.channel.send({ files: ["You can react with custom emojis!"] });
